;;; packages.el --- my-keyboard-layout layer packages file for Spacemacs.
;;
;; Copyright (c) 2012-2016 Sylvain Benner & Contributors
;;
;; Author: tzskp1 <tzskp1>
...
;; added to `my-keyboard-layout-packages'. Then, for each package PACKAGE:
;;
;; - If PACKAGE is not referenced by any other Spacemacs layer, define a
;;   function `my-keyboard-layout/init-PACKAGE' to load and initialize the package.

;; - Otherwise, PACKAGE is already referenced by another Spacemacs layer, so
;;   define the functions `my-keyboard-layout/pre-init-PACKAGE' and/or
;;   `my-keyboard-layout/post-init-PACKAGE' to customize the package as it is loaded.

;;; Code:
(defconst my-keyboard-layout-packages
  '(
    ace-window
    avy
    comint
...
(defun my-keyboard-layout/pre-init-ace-window ()
...
(defun my-keyboard-layout/pre-init-avy ()
  (kl|config avy
    :description
...
(defun my-keyboard-layout/pre-init-comint ()
...
(defun my-keyboard-layout/pre-init-company ()
  (kl|config company
    :description
...
(defun my-keyboard-layout/pre-init-elfeed ()
  (kl|config elfeed
    :description
...
(defun my-keyboard-layout/pre-init-evil ()
  (kl|config evil
    :description
    "Remap `evil' bindings."
    :loader
...
(defun my-keyboard-layout/pre-init-evil-escape ()
...
(defun my-keyboard-layout/pre-init-evil-evilified-state ()
...
(defun my-keyboard-layout/pre-init-evil-surround ()
...
(defun my-keyboard-layout/pre-init-eyebrowse ()
  (kl|config eyebrowse
    :description
...
(defun my-keyboard-layout/pre-init-flycheck ()
...
(defun my-keyboard-layout/pre-init-helm ()
  (kl|config helm
    :description
    "Remap `helm' bindings."
    :loader
  ...
  (kl|config helm-files
	...
	:dvorak
	(progn
      (kl/set-in-state helm-find-files-map "C-t" 'helm-previous-line)
      (kl/set-in-state helm-find-files-map "C-h" 'helm-next-line)
      (kl/set-in-state helm-read-file-map "C-t" 'helm-previous-line)
      (kl/set-in-state helm-read-file-map "C-h" 'helm-next-line))
	)
...
(defun my-keyboard-layout/pre-init-imenu-list ()
...
(defun my-keyboard-layout/pre-init-ivy ()
  (kl|config ivy
    :description
    "Remap `ivy' bindings."
    :loader
...
(defun my-keyboard-layout/pre-init-magit ()
  (kl|config magit
    :description
...
(defun my-keyboard-layout/pre-init-mu4e ()
  (kl|config mu4e
    :description
...
(defun my-keyboard-layout/pre-init-neotree ()
  (kl|config neotree
    :descripition
...
(defun my-keyboard-layout/pre-init-org ()
  (kl|config org
    :description
...
(defun my-keyboard-layout/pre-init-org-agenda ()
...
(defun my-keyboard-layout/pre-init-ranger ()
  (kl|config ranger
    :description
